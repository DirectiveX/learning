            ...
            required: function(value, element, param) {
                ...
                switch (element.nodeName.toLowerCase()) {
                    case 'select':
                        ...
                        return options.length > 0 && (element.type == "select-multiple" || (navigator.userAgent.match(/msie/i) && !(options[0].attributes['value'].specified) ? options[0].text : options[0].value).length > 0);
                    case 'input':
                        if (this.checkable(element))
	...
	$.each({
		...
		$.event.special[fix] = {
			...
			handler: function(e) {
				...
				return $.event.dispatch(this, arguments);
			}
		};
	});
	$.extend($.fn, {
