...
// @return N/A
//
menubar.prototype.doMenu = function (e) {
    var item = this.$parents.first();
    item.focus();
    this.hideMenuSiblings(item);
    e.returnValue = false;
    e.cancelBubble = true;
        ...
        case this.keys.pageup:
            {
                ...
                if (this.vmenu == true && $itemUL.is('.root-level')) {
                    ...
                    this.$activeItem = this.moveToPrevious($item, e.keyCode != this.keys.up);
                }
                else {
                    this.$activeItem = this.moveUp($item, e.keyCode != this.keys.up);
                }
        ...
        case this.keys.pagedown:
            {
                ...
                if (this.vmenu == true && $itemUL.is('.root-level')) {
                    ...
                    this.$activeItem = this.moveToNext($item, e.keyCode != this.keys.down);
                }
                else {
                    this.$activeItem = this.moveDown($item, null, e.keyCode != this.keys.down);
                }
...
//
menubar.prototype.moveToNext = function ($item, noDownKey) {
...
// @return (object) Returns the item to move to. Returns $item is no move is possible
//
menubar.prototype.moveToPrevious = function ($item, noUpKey) {
...
// @return (object) Returns the item to move to. Returns $item is no move is possible
//
menubar.prototype.moveDown = function ($item, startChr, noDownKey) {
    ...
    if ($itemUL.is('.root-level')) { // this is the root level menu
        ...
        if (noDownKey) {
            return $item.parent().children('.menu-parent').last();
        }
...
// @return (object) Returns the item to move to. Returns $item is no move is possible
//
menubar.prototype.moveUp = function ($item, noUpKey) {
    ...
    if ($itemUL.is('.root-level')) { // this is the root level menu
        ...
        if (noUpKey) {
            return $item.parent().children('.menu-parent').first();
        }
        // nothing to do
        return $item;
    }
...
menubar.prototype.hideMenuSiblings = function ($item) {
    var menusiblings = $item.siblings('.menu-parent');
    for (var i = 0; i < menusiblings.length; i++) {
        menusiblings.removeClass('menu-focus menu-focus-checked menu-hover menu-hover-checked');
        menusiblings.mouseleave();
    }
}
//
...
menubar.prototype.processMenuChoice = function ($item) {
    ...
    var choice = $item.attr('targeturl');
