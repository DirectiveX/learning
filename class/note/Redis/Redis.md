...
问题：为什么是过半？
因为如果不是过半，可能产生2组相同投票结果进行竞争，产生脑裂（网络分区）问题，网络分区会导致相同请求，不同返回结果的问题，如果过半，就没有这种问题。一般的，我们会使用奇数台机器做备用服务器，因为奇数台机器和偶数台机器首先成本上奇数台机器相对较低，并且奇数台机器的容错率比偶数台高，拿5台备机和4台备机相比，投票数都是3，但是4台备机只允许一台出错，而五台备机允许两台出错，这是一个分区容忍性的问题。
