...
如果使用到了java内置的接口如JDBC，那么遵循双亲委派的做法是采用Class.forName通过当前类的ClassLoader进行加载，但是如果通过ClassLoader，那么加载DriverManager的ClassLoader必然就是BootStrapClassLoader，而加载实现类肯定用不了BootStrapClassLoader，只能用自己实现的ClassLoader或者AppClassLoader，所以要向下寻找，spi使用了getContextClassLoader()方法去找到下面的ClassLoader进行一个加载
