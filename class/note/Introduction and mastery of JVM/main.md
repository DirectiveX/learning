...
并发标记：
1.将整个内存空间的地址视图设置位Remapped
2.如果访问的对象是Remmapped，改成M0表示被标记，如果是M0（被其他线程标记或者新创建的对象）则不处理（写屏障会无脑将扫过的对象设置为M0）。结束后会将所有的M0活跃对象存入对象活跃信息表

最终标记（STW）
并发预备重分配（Concurrent Prepare for relocate）:根据条件找出要清理的region，放入Relocation Set中，处理弱引用和类卸载

并发重分配：将重分配集中的存活对象复制到新的region上，为重分配集中每个region维护转发表（为了当用户线程访问那些被转移的对象时，通过转发表将旧地址转换成新地址访问，并且同时修改引用值让引用指向新对象，自愈能力）

并发重映射（Concurrent Remap）：修正重分配集中所有旧对象的引用，修正完毕，对转发表进行释放

ps：重分配集表示的是在这之内的region中的存活对象要被转移
M0和M1的关系对标S0和S1，下一次使用的时候会用到（其实就是处理浮动垃圾）
