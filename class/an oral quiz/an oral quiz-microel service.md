...
检验账号密码成功，通过JWT用用户uuid生成token，设置过期时间为-1，然后再把token放入redis中，过期时间设置为30分钟（毕竟是政府项目，过期太晚不安全，而且平时操作的时候也没那么久，但是如果快过期了会进行弹窗提示，用户可以点extends来续约，重复登陆直接报多点登陆异常）返回token，前端请求的时候在header里带上token进行请求

修改密码或者退出会删除redis里的token

# 数据库的分布式锁

注意，用的较少，必须单库，用唯一键存一条锁记录

# 主从集群redis分布式锁的问题

当前线程向主加锁拿到锁，此时主挂了还没向从发送数据，故障修复，从变主，此时其他线程加锁成功，造成同时获得锁

解决方案：

1.红锁（多设几个redis进行加锁，加锁的时候设置wait，过半通过）

## 红锁的缺点

如果现在有5台redis编号12345，线程A获取锁123，然后此时3挂了，运维立马把3起了，此时线程A业务未执行完，线程B去获取锁，获得345，导致B加锁成功，造成同时获得锁

解决方法：部署手册：延时启动

## 未解决问题（ZK和Redis都会有问题）

jvm获取锁之后stw了，线程停止工作，不续约，导致key过期，其他线程获得锁，造成同时获得锁

解决方法：

1.鸵鸟算法

2.（zk顺序节点+mysql）存的时候存一下数据库，stw之后需要回查数据库，如果找不到存的数据就代表被其他线程锁住了，就不执行了

# 2PC如果最后一阶段失败

开启定时任务进行补偿

协调者（lcn（占用连接）或者seataAT（不占用连接资源，它做好了镜像记录（在seataAT服务本地有个undo_log），在一阶段已经落库了））

## 2PC的问题

单点故障，占用资源，数据不一致需要人工补偿

# 3PC

解决2PC的占用资源的问题，解决2PC中协调者宕机，参与者自己有超时机制，在2PC中参与者没有超时机制，目前没有落地方案，面试官傻逼才问
